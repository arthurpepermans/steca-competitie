...
export function SupporterProfielBadges(){const {supporter}=useAuth();const info=useFans();const rij=info.data&&berekenFans(info.data,null).find(p=>p.user_id===supporter?.user_id);return <><Fout tekst={info.fout}/>{info.laden?<Laden/>:rij?<FanProfiel rij={rij}/>:<p>Nog geen supportersprofiel gevonden.</p>}<p><Link className="knop" to="/supporter-klassement">Supportersklassement</Link></p></>;}
