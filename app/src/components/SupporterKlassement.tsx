import {SupporterFunctieBeheer} from './SupporterBeheer';
...
function useFans(persoon?:string){const {lid}=useAuth();const beheer=rechten(lid).isAdmin;const [v,setV]=useState(0);useEffect(()=>{const f=()=>setV(x=>x+1);window.addEventListener('supporter-badges',f);return()=>window.removeEventListener('supporter-badges',f);},[]);return useAsync(async()=>{const {data,error}=persoon&&beheer?await supabase.rpc('admin_supporter_profiel',{p_id:persoon}):await supabase.rpc('supporter_klassement_data');if(error)throw error;return data as FanData;},[v,persoon,beheer]);}
...
function FanProfiel({rij}:{rij:ReturnType<typeof berekenFans>[number]}){return <section className="kaart"><h2>{rij.naam}</h2><h3>Statistieken</h3><div className="supporter-profiel-tellingen"><p><strong>{rij.perSeizoen[seizoenNu()] ?? 0}</strong><span>Dit seizoen ({seizoenNu().replace('-','/')})</span></p><p><strong>{rij.matchen}</strong><span>Aller tijden</span></p><p><strong>{rij.uit}</strong><span>Uitmatchen · aller tijden</span></p></div><h3>Supportersbadges</h3>{!rij.badges.length&&<p>Nog geen badges verdiend.</p>}<div className="badge-test-grid">{rij.badges.map(b=>{const badge=FAN_BADGES.find(x=>x.id===b.badge);return badge?<article className="badge-test-kaart" key={b.badge+(b.seizoen??'')}><SupporterBadgeIcoon badge={badge}/><strong>{badge.titel}</strong>{b.seizoen&&<small>{b.seizoen.replace('-','/')}</small>}<span>{badge.uitleg}</span></article>:null;})}</div></section>;}
...
export function SupporterDetail(){
 const {id}=useParams();const {lid}=useAuth();const info=useFans(id);
 const rij=info.data&&berekenFans(info.data,null).find(p=>p.user_id===id);
 return <><p><Link to={rechten(lid).isAdmin?'/leden?tab=supporters':'/supporter-klassement'}>← Terug naar {rechten(lid).isAdmin?'supporters beheren':'supportersklassement'}</Link></p><h1>Supportersprofiel</h1><Fout tekst={info.fout}/>{info.laden?<Laden/>:rij?<><FanProfiel rij={rij}/>{rechten(lid).isAdmin&&<SupporterFunctieBeheer id={rij.user_id!} naam={rij.naam} actief={rij.actief !== false}/>}</>:!info.fout&&<p>Supporter niet gevonden of geen toegang.</p>}</>;
}
