...
export function SupporterFunctieBeheer({id,naam,actief=true}:{id:string;naam:string;actief?:boolean}){
 ...
 async function account(actie:string){
  if(actie==='verwijderen' && prompt(`Supporteraccount van ${naam} definitief verwijderen? Aanwezigheden, pronostieken, Dream XI en bijbehorende supportersbadges verdwijnen. Typ VERWIJDER om te bevestigen.`)!=='VERWIJDER')return;
  setBezig(true);setFout(null);try{const {error}=await supabase.rpc('admin_supporter_account',{p_id:id,p_actie:actie});if(error)throw error;if(actie==='verwijderen')navigate('/leden?tab=supporters');else window.dispatchEvent(new Event('supporter-badges'));}catch(e){setFout(foutTekst(e));}finally{setBezig(false);}
 }
 return <section className="kaart"><h2>Functie aanpassen</h2><div className="knoppen"><select aria-label={'Functie van '+naam} value={functie} disabled={bezig} onChange={e=>setFunctie(e.target.value as Functie)}>{FUNCTIES.map(f=><option key={f} value={f}>{FUNCTIE_LABEL[f]}</option>)}</select><button className="knop klein" disabled={bezig||functie==='supporter'} onClick={async()=>{setBezig(true);setFout(null);try{const memberId=await veranderAccountfunctie(id,functie);navigate('/leden/'+memberId);}catch(e){setFout(foutTekst(e));}finally{setBezig(false)}}}>Functie opslaan</button></div><h2>Account beheren</h2><p>{actief?'Dit supporteraccount is actief.':'Dit supporteraccount is gedeactiveerd.'}</p><div className="knoppen"><button className="knop licht" disabled={bezig} onClick={()=>account(actief?'deactiveren':'activeren')}>{actief?'Deactiveren':'Opnieuw activeren'}</button><button className="knop gevaar" disabled={bezig} onClick={()=>account('verwijderen')}>Account definitief verwijderen</button></div><p className="klein zacht">Bij deactiveren blijven de gegevens bewaard. Definitief verwijderen kan niet ongedaan worden gemaakt.</p><Fout tekst={fout}/></section>;
}
