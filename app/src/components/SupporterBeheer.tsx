...
function SupporterRij({supporter:s}:{supporter:Supporter}){
 ...
 return <li><Link className="tekst-knop" to={'/supporter-profiel/'+s.id}><strong>{s.naam}</strong></Link><p className="klein zacht">{s.heeft_account?'Supporteraccount':'Zonder account'}{!s.actief?' · inactief':''}</p><div className="knoppen"><select aria-label={'Functie van '+s.naam} value={functie} disabled={bezig} onChange={e=>setFunctie(e.target.value as Functie)}>{FUNCTIES.map(f=><option key={f} value={f}>{FUNCTIE_LABEL[f]}</option>)}</select><button className="knop klein" disabled={bezig||functie==='supporter'} onClick={async()=>{setBezig(true);setFout(null);try{const id=await veranderAccountfunctie(s.id,functie);navigate('/leden/'+id);}catch(e){setFout(foutTekst(e));}finally{setBezig(false)}}}>Functie opslaan</button></div><Fout tekst={fout}/></li>;
}
