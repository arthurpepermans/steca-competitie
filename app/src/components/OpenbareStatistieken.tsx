...
export function OpenbareStatistieken({ matches, boetepot = false }: { matches: Match[]; boetepot?: boolean }) {
  ...
  return <>
    ...
    <div className="tabel-wrap"><table className="tabel"><thead><tr><th>#</th><th>Speler</th>{kolommen.map(k=><th key={k.veld} aria-sort={k.veld===sorteer ? "descending" : undefined}>{k.label}</th>)}</tr></thead><tbody>
      {rijen.map((r, i) => <tr key={r.member_id}><td>{i + 1}</td><td style={{ whiteSpace: "normal" }}>{namen.get(r.member_id) ?? "Speler"}</td>{kolommen.map(k=><td key={k.veld}>{k.veld===sorteer ? <strong>{r[k.veld]}</strong> : r[k.veld]}</td>)}</tr>)}
