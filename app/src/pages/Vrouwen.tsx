...
export function Vrouwen() {
  ...
  const [berichten,setBerichten]=useState<ClubBericht[]>([]);
  useEffect(()=>{let actief=true;const laad=()=>{void Promise.resolve(supabase.rpc('club_lichtkrant',{p_club:'vrouwen'})).then(({data,error})=>{if(actief&&!error)setBerichten(data??[]);});};laad();const timer=setInterval(laad,60000);window.addEventListener(VROUWEN_BAND,laad);return()=>{actief=false;clearInterval(timer);window.removeEventListener(VROUWEN_BAND,laad);};},[]);
  ...
  return <>
    ...
    <main className="inhoud v-inhoud">{typeof document!=='undefined'&&document.documentElement.dataset.installatie==='vrouwen'&&<InstallatieHulp wegklikbaar/>}{(tab==='klassement'||tab==='statistieken')&&<div className="tabs"><Link className={tab==='klassement'?'actief':''} to="/vrouwen/klassement">Klassement</Link><Link className={tab==='statistieken'?'actief':''} to="/vrouwen/statistieken">Statistieken</Link></div>}{clubActief&&<VrouwenClub kalenderBron={data} key={pathname} tab={tab} detail={pathname.split('/')[3]} stand={data?.klassementen[0]?.rijen.find(r=>r.naam==='STECA VROUWEN')} />}
